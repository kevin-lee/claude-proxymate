...
object JsonTreeView {
  ...
  private def buildStringFrag(s: String, trailing: Frag, path: String): Frag = {
    if (s.length > CollapseLenThreshold) {
      ...
      if (hits.isEmpty) {
        ...
        val parts =
          buildTokenizedSlice(s, hits, sliceOffset = 0, path, flattenNewlines = false).getOrElse(List(stringFrag(s)))
  ...
  /** A merged mask candidate (regex-token from PR 2 OR
    ...
    * suffices.
    */
  sealed private trait MaskHit { def start: Int; def end: Int }
  final private case class TokenHit(start: Int, end: Int) extends MaskHit
  final private case class CorrHit(start: Int, end: Int, name: String) extends MaskHit
  ...
  private def buildTokenizedSlice(
    ...
    if (inSlice.isEmpty) return none[List[Frag]]
  ...
  private def buildContainerFrag(
    ...
    val entries: js.Array[(String | Null, js.Dynamic)] =
      if (isArr) {
        val arr = value.asInstanceOf[js.Array[js.Dynamic]]
        arr.map { v => (null, v): (String | Null, js.Dynamic) }
      } else {
        val keys = js.Object.keys(value.asInstanceOf[js.Object])
        keys.map { k => (k, value.selectDynamic(k)): (String | Null, js.Dynamic) }
      }
    ...
    val rowFrags: List[Frag] = entries.toList.zipWithIndex.map {
      case ((k, v), i) =>
        AppState.jtLine += 1
