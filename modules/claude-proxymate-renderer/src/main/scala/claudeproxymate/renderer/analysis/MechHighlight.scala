...
object MechHighlight {
  ...
  private def expandLongStr(el: dom.Element): Unit = {
    ...
    if (
      target.classList.contains("jt-str-expanded") && target.asInstanceOf[dom.html.Element].style.display === "none"
    ) {
    ...
    if (wholeElement) {
      ...
      locally {
        val _ = dom.window.requestAnimationFrame { _ =>
          locally {
            val _ =
              el.asInstanceOf[js.Dynamic].scrollIntoView(js.Dynamic.literal("behavior" -> "smooth", "block" -> "start"))
          }
        }
      }
      return
    ...
    if (hl != null) {
      locally {
        val _ = dom.window.requestAnimationFrame { _ =>
          locally {
            val _ =
              hl.asInstanceOf[js.Dynamic].scrollIntoView(js.Dynamic.literal("behavior" -> "smooth", "block" -> "start"))
          }
        }
      }
    }
            ...
            val _ = dom.window.requestAnimationFrame { _ =>
              locally {
                val _ = itemRow
                  ...
                  .scrollIntoView(js.Dynamic.literal("behavior" -> "smooth", "block" -> "start"))
              }
            }
          }
          return
