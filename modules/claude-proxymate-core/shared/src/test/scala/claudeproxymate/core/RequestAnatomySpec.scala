...
object RequestAnatomySpec extends Properties {
  ...
  def testEstTokensSeven: Property =
    for { _ <- Gen.constant(()).forAll } yield
      Result.assert(RequestAnatomy.estTokens(7) == 2).log(s"got ${RequestAnatomy.estTokens(7)}")
  ...
  def testStructureRoles: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      val body = Json.obj(
        "messages" -> Json.arr(userMsg("a"), assistantThinking("s"), userMsg("b"), assistantThinking("s2"), userMsg("c"), userMsg("d"), assistantThinking("s3")),
      )
      val s = analyze(body).structure
      Result.assert(s.messageCount == 7).log(s"messageCount=${s.messageCount}") and
        Result.assert(s.userTurns == 4).log(s"userTurns=${s.userTurns}") and
        Result.assert(s.assistantTurns == 3).log(s"assistantTurns=${s.assistantTurns}")
    }
    ...
    for { _ <- Gen.constant(()).forAll } yield {
      val body = Json.obj(
        "system" -> Json.arr(
          ...
          Json.obj("type" -> "text".asJson, "text" -> "id".asJson, "cache_control" -> Json.obj("type" -> "ephemeral".asJson)),
          Json.obj("type" -> "text".asJson, "text" -> "main".asJson, "cache_control" -> Json.obj("type" -> "ephemeral".asJson)),
        ),
        "messages" -> Json.arr(userMsg("hi")),
      )
      val s = analyze(body).structure
      Result.assert(s.systemBlocks == 3).log(s"systemBlocks=${s.systemBlocks}") and
        Result.assert(s.cachedSystemBlocks == 2).log(s"cached=${s.cachedSystemBlocks}")
    }
  ...
  def testStructureBlocks: Property =
    for { _ <- Gen.constant(()).forAll } yield {
        ...
        "messages" -> Json.arr(
          Json.obj(
            ...
            "content" -> Json.arr(
              ...
              Json.obj("type" -> "tool_use".asJson, "name" -> "Skill".asJson, "id" -> "u1".asJson, "input" -> Json.obj()),
            ),
          ),
          Json.obj(
            "role" -> "user".asJson,
      ...
      val s = analyze(body).structure
      Result.assert(s.toolsDefined == 1).log(s"tools=${s.toolsDefined}") and
        Result.assert(s.toolUseCount == 1).log(s"toolUse=${s.toolUseCount}") and
        Result.assert(s.toolResultCount == 1).log(s"toolResult=${s.toolResultCount}") and
        Result.assert(s.imageCount == 1).log(s"images=${s.imageCount}") and
        Result.assert(s.thinkingBlocks == 1).log(s"thinking=${s.thinkingBlocks}")
    }
  ...
  def testLatestAndHistory: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      ...
      Result.assert(latest.exists(_.bytes > 0)).log(s"latest=$latest") and
        Result.assert(history.forall(_.bytes >= 0)).log(s"history=$history")
    }
  ...
  def testSegmentsSorted: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      ...
      Result.assert(sortedOk).log(s"not sorted: $segs") and
        Result.assert(segs.forall(_.bytes > 0)).log(s"zero-byte row present: $segs")
    }
  ...
  def testSecurityAnomaly: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      ...
      Result.assert(hasSec(both)).log("should fire with both") and
        Result.assert(!hasSec(credOnly)).log("should not fire cred-only") and
        Result.assert(!hasSec(echoOnly)).log("should not fire echo-only")
    }
  ...
  def testWarnAnomalies: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      ...
      Result.assert(noResp.exists(a => a.kind == AnomalyKind.Warn && a.key == "anatomy.noResponseNote")).log(s"noResp=$noResp") and
        Result.assert(trunc.exists(a => a.kind == AnomalyKind.Warn && a.key == "anatomy.anomTruncated")).log(s"trunc=$trunc")
    }
  ...
  def testEmptyBody: Property =
    for { _ <- Gen.constant(()).forAll } yield {
      ...
      Result.assert(a.segments.isEmpty).log(s"segments=${a.segments}") and
        Result.assert(a.structure.messageCount == 0).log("messageCount") and
        Result.assert(a.inventory.isEmpty).log("inventory") and
        Result.assert(a.anomalies.isEmpty).log(s"anomalies=${a.anomalies}")
    }
}
